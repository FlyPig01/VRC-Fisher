...
public sealed class FishingStateMachineTests
{
    ...
    [Fact]
    public void Confirmed_success_ends_the_minigame()
    {
        var options = StateMachineOptions.Default with
        {
            PromptConfirmFrames = 1,
            UiConfirmFrames = 1,
            SuccessConfirmFrames = 1
        };
        var machine = new FishingStateMachine(options);
        var now = DateTimeOffset.UtcNow;
        machine.Step(new DetectionObservation(1, now), now);
        ...
        var success = machine.Step(new DetectionObservation(
            5,
            now,
            FishingUi: new BoundingBox(0, 0, 100, 100),
            Success: new BoundingBox(0, 0, 10, 10)), now.AddSeconds(1.4));

        Assert.Equal(FishingPhase.Reeling, success.Phase);
    }
}
