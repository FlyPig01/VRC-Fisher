...
最终独立审核产物为 `training/test/results/屏幕录制 2026-08-14 225423-onnx-review.mp4`。输入录屏没有出现在两个训练数据 `split.json`、训练数据文件名或数据处理输出中；此前的 `感叹号验证集` 和 `屏幕录制 2026-08-12 223804` 已删除审核视频，因为它们参与过训练/验证流程，不再作为独立测试。新视频共 3048 帧、30 FPS；使用 `--inference-stride 3` 每 3 帧推理一次，输出容器保持原帧数、帧率和时长，但每组后两帧复用最近一张已标注画面，视觉更新约为 10 Hz。共得到 1016 个推理帧、394 个 locator 检测和 486 个 minigame 检测。该结果只用于人工检查框位置、连续性和明显漏检，不能计算逐帧 mAP、Precision 或 Recall。发布前仍需人工审核本视频，并进行未参与训练/阈值调整的现场“仅观察”测试；没有正式模型前，任何自动钓鱼成功率都不能写成结果。
