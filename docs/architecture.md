...
WGC 适配器使用独立 `DispatcherQueue` 线程。`GraphicsCaptureItem`、D3D 设备、帧池和捕获会话在同一线程创建、启动、停止和释放，避免 WinRT 对象跨线程访问。自由线程帧池取得画面后完成 BGRA 读回，跨层只传递普通字节、尺寸和捕获时间，不传递 WinRT 或 GPU 资源对象。
...
运行模式只记录状态和错误；调试模式增加推理、检测框、控制决策，以及带独立操作编号的收杆/抛竿时间线。每行日志记录托管线程编号；WGC 故障另外记录操作阶段和 HRESULT，用于区分线程、捕获与输入问题。每种模式只保留当前和上一会话，总目录上限为 `2.5 MiB`。
