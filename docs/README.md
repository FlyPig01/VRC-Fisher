...
当前已实现 C# 分层工程、状态机、连续帧证据、鼠标安全释放、F8 紧急停止、最新帧缓冲、模型清单/校验/事务下载基础、CPU Provider 入口、YOLO 常见输出解码、WinUI 系统捕获选择器、D3D11/WGC CPU readback 和 7 项 C# 自动化测试。正式标注数据、两个有效 ONNX、真实 VRChat 场景验收、完整模型下载 GUI 和 C# 打包链仍未完成。当前自动模式必须在正式模型契约与现场验证完成后才可发布；没有人工标注时不能训练或报告准确率。`app/` 中的旧 Python 原型与 `packaging/` 中的旧 PyInstaller 脚本只能用于逻辑参考，不代表正式运行时。
