...
| 训练 | Python 3.11、PyTorch、Ultralytics YOLO11、ONNX | 当前先用 CPU 跑通；耗时不可接受时再决定 CUDA |

训练保存 `.pt` 检查点和实验结果，只有审核后的模型才导出为 ONNX。`.pt` 便于继续训练，但要求 Python/PyTorch 运行环境，不适合作为用户端格式。

是否用 CUDA 只影响开发机训练速度，不影响最终用户选择 CPU-only 或 DirectML，也不改变模型类别和 ONNX 契约。以后启用 CUDA 时，应根据当时的 NVIDIA 驱动和 PyTorch 官方兼容矩阵安装，不能把一套固定 CUDA 命令写成通用环境。

## 5. 当前开发机

截至 2026-08-13，本机只读检查结果：
